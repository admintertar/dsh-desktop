...
## Workbench shared theme

The opt-in Workbench launcher owns one shared `light` / `dark` / `system` preference under its application userData (`workbench-theme/state.json`). Its private, optional `desktopRuntime.sharedTheme` capability synchronizes only `ui-theme.preference` through each Host's official settings service before mounting the Renderer, and updates native materials through `nativeTheme.themeSource`. The first connected Host seeds a missing shared preference; later Hosts, Profile switches and project recovery inherit it. Closing a project does not restore an earlier native theme. Font size and other Profile settings remain independent, and ordinary standalone Desktop launches keep their existing behavior. Plugins should use the official Appearance control, not this launcher-private capability or custom sidebar backdrops.
